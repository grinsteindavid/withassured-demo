...
- **Vercel Workflows** — `lib/workflow/` uses the Vercel Workflow SDK (`'use workflow'` / `'use step'`) backed by a Prisma `Workflow` table. Compliance checks are driven by Vercel Cron.
...
| [`docs/workflows.md`](docs/workflows.md) | Vercel Workflow SDK setup, database-backed steps, cron configuration, and read paths. |
